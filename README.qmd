...
signif_posthoc_bycond <- significant_posthoc |>
        filter(grepl("Condition",test)) |>
        select(variable)

signif_posthoc_bysite <- significant_posthoc |>
...
#contrast by condition at each site
summary_bycond_df <- 
        plot_df |>
        ...
        dplyr::mutate(
                significant=ifelse(
                        site=="ROOF" & Attribute %in% signif_posthoc_bycond,
                        TRUE,
                        FALSE)
                ) |>
        pivot_wider(
        ...
        dplyr::mutate(
                AMSS = paste0(
                        formatC(mean_AMSS,format = "f",digits = 2),
                        " (",
                        formatC(sd_AMSS,format = "f",digits = 2),
                        ")"
                ),
                AMB = paste0(
                        formatC(mean_AMB,format = "f",digits = 2),
                        " (",
                        formatC(sd_AMB,format = "f",digits = 2),
                        ")"
                ), 
                difference = formatC(
                        (mean_AMSS-mean_AMB)/2*100,
                        format = "f",digits = 2
                        ),
                .keep = "unused"
        ) |>
        dplyr::select(!c("significant")) |>
        pivot_wider(
                names_from = "site",
                values_from = c("AMB","AMSS","difference"),
                names_glue = "{site}_{.value}"
        ) |>
        dplyr::select(c("Attribute",
                        "GND_AMB","GND_AMSS","GND_difference",
                        "ROOF_AMB","ROOF_AMSS","ROOF_difference")) |>
        gt(rowname_col = "Attribute") |>
        tab_spanner_delim(delim = "_")
summary_bycond_df
```

Extended Data Table 2:Summary of statistical tests for attributes in soundscape evaluation questionnaire (sound source dominance, overall quality, appropriateness, loudness, revisit frequency, *ISOPL*, *ISOEV*, and PRSS dimensions) across site (GND and ROOF), condition (AMSS and AMB), and their interaction (*site*:*condition*). Test abbreviations and symbols for significance levels and effect sizes are defined in the footnote.
